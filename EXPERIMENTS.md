...
5. **Learn the next skill.** MiniLM text tokens, arm height, jaw state, and completed-skill counts fed a GRU action head. MuJoCo demonstrations supplied labels. The saved v10 head's held-out *step-label* score was **5,238/5,502** on generated language/state examples. That score does not measure full physical completion.
6. **Check complete behavior.** On the recorded v10 simulation checks, **8/8** approach-only commands and **16/16** supported mixed commands finished with the expected action sequence and checked physical outcome. These are small, specific held-out suites, not a claim about arbitrary language or objects.
7. **Stress and revise without task-specific training.** A frozen-policy stress test passed **16/28** unfamiliar-object and environment trials. The current demo uses a same-camera empty-table RGB reference, frozen SigLIP 2 crop ranking, and a frozen Qwen3 planner over the existing physical skills. The original trained heads remain in the repo for comparison, but the current demo does not load them. See [research](docs/robustness-research.md) and the [revised evaluation](eval/ood-v2/REPORT.md).
...
- The earlier `unsupported` action produced a label for unfamiliar commands. It was removed. The current planner can produce an empty plan for “dance for me”; that is the frozen model's output, not a hardcoded rejection.
...
| `so100/language_planner.py`, `so100/action_head.py` | Current frozen planner, historical head, and executable skills |
...
| `so100/train_action_head.py`, `so100/train_action_target.py` | Historical head training and generated training examples |
...
The four included `.pt` files preserve the historical target/action heads and their immediate warm-start checkpoints. The much larger generated datasets and abandoned experiment branches remain outside this focused repo.
